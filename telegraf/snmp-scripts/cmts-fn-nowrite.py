...
def main(argv):
   ...
   sName = sysName(agent_host,community)
   ...
   for row in result:
       ...
       for f in findex:
           ...
           for s in sindex:
               ...
               if ioid != '0':
#                   aliasOid = ("%s.%s" % (ifAlias,ioid))
#                   alias = get(agent_host,community,aliasOid)
#                   alias = alias.decode("utf-8")
                   print ("%s,agent_host=%s,host=%s,hostname=%s,ifIndex=%s,fnDescr=%s index=%s" % (mn,agent_host,host,sName,ioid,fnName,ioid))
               else:
                   foid = foid.replace('.','')
                   print ("%s,agent_host=%s,host=%s,hostname=%s,ifIndex=%s,fnDescr=%s index=%s" % (mn,agent_host,host,sName,foid,fnName,foid))
